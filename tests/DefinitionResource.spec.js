import { buildAPIEndpoint } from '@/index'
...
describe('test actions creation based on last parameter', () => {
  ...
  test('resource definition', () => {
    const items = buildAPIEndpoint({ http: http, baseURL: API_URL, resourcePath: path })
  ...
  test('resource single urls list with subresource', () => {
    const items = buildAPIEndpoint({ http: http, baseURL: API_URL, resourcePath: path }).addSubresource('images/')
    expect(items.single(uuid1)).toBeDefined()
