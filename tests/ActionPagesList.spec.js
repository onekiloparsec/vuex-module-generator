...
const routes = [
  {
    method: 'GET',
    url: 'items/?page=1',
    response: { body: { results: [mock1, mock2], count: 7, previous: null, next: API_URL + 'items/?page=2' }, status: 200 }
  },
  {
    method: 'GET',
    url: 'items/?page=2',
    response: { body: { results: [mock3, mock4], count: 7, previous: API_URL + 'items/?page=1', next: API_URL + 'items/?page=3' }, status: 200 }
  },
  {
    method: 'GET',
    url: 'items/?page=3',
    response: { body: { results: [mock5, mock6], count: 7, previous: API_URL + 'items/?page=2', next: API_URL + 'items/?page=4' }, status: 200 }
  },
  {
    method: 'GET',
    url: 'items/?page=4',
    response: { body: { results: [mock7], count: 7, previous: API_URL + 'items/?page=3', next: null }, status: 200 }
  }
]
...
describe('test async api actions on module directly', () => {
  ...
  beforeEach(() => {
    itemsModule = makeStoreModule({
      ...
      root: 'item',
      idKey: 'uuid',
      lcrusd: 'pcrusd', // NOTE THE P!!! (instead of the l)
...
describe('test async api actions inside a valid store', () => {
  ...
  beforeEach(() => {
    const itemsModule = makeStoreModule({
      http: Vue.http,
      apiURL: API_URL,
      apiPath: 'items/',
      root: 'item',
